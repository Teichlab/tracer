...
* `-s/--species` : Species from which the T cells were derived. Options are `Mmus` or `Hsap` for mouse or human data. This is important for determination of invariant cells in the `summarise` step because it defines the V segments that are indicative of these cells. If you have defined new species using the [`build`](#build) mode, you should specify the same name here. Default = `Mmus`.
* `--receptor_name` : If, for some reason, you've used a different receptor name when using [`build`](#build) then you should also specify it here. Default = `TCR`
* `--loci` : The specific loci that you wish to assemble. TraCeR knows about alpha (`A`), beta (`B`), gamma (`G`) and delta (`D`) for mouse and human TCRs. Other locus names can be specified if you use [`build`](#build). By default, TraCeR will attempt to assemble alpha and beta sequences. To change this, pass a space-delimited list of locus names. For example, to only look for gamma/delta sequences use `--loci G D`. **Important** The way that this argument is parsed means that you **must** follow it with another command line option or the argument parser will get confused. For example: `--loci G D --p 1`. We know this [isn't ideal](https://github.com/Teichlab/tracer/issues/62).
...
* `--resource_dir`: the directory containing the resources required for alignment. By default this is the `resources` directory in this repository, but can be pointed to a user-built set of resources.
* `-s/--species` : Species from which the T cells were derived. Options are `Mmus` or `Hsap` for mouse or human data. This is important for determination of invariant cells because it defines the V segments that are indicative of these cells. If you have defined new species using the [`build`](#build) mode, you should specify the same name here. Default = `Mmus`.
* `--receptor_name` : If, for some reason, you've used a different receptor name when using [`build`](#build) then you should also specify it here. Default = `TCR`
* `--loci` : The specific loci that you wish to summarise. TraCeR knows about alpha (`A`), beta (`B`), gamma (`G`) and delta (`D`) for mouse and human TCRs. Other locus names can be specified if you use [`build`](#build). By default, TraCeR will attempt to summarise alpha and beta sequences. To change this, pass a space-delimited list of locus names. For example, to only look for gamma/delta sequences use `--loci G D`. **Important** The way that this argument is parsed means that you **must** follow it with another command line option or the argument parser will get confused. For example: `--loci G D --p 1`. We know this [isn't ideal](https://github.com/Teichlab/tracer/issues/62). Default = `A B`
