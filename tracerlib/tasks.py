...
class Assembler(TracerTask):
    ...
    def quantify(self, cell):
        ...
        if not self.config.has_option('base_transcriptomes', self.species):
            raise OSError(
                "No transcriptome reference specified for {species}. Please specify location in config file."
                .format(species=self.species))
        else:
            base_transcriptome = self.resolve_relative_path(
                self.config.get('base_transcriptomes',
                                self.species))
        ...
        if self.quant_method == 'salmon':
            ...
            tracer_func.quantify_with_salmon(salmon, cell, self.output_dir,
                                             self.cell_name, base_transcriptome,
                                             self.fastq1, self.fastq2,
                                             self.ncores,
                                             self.resume_with_existing_files,
                                             self.single_end,
                                             self.fragment_length,
                                             self.fragment_sd, salmon_libType,
                                             salmon_kmerLen)
            print()
            ...
            tracer_func.quantify_with_kallisto(kallisto, cell, self.output_dir,
                                               self.cell_name,
                                               base_transcriptome,
                                               self.fastq1, self.fastq2,
                                               self.ncores,
